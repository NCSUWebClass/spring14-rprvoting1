...
function candidateByID($link, $id){
    ...
    $sth = mysql_query("SELECT * FROM csc342.positions WHERE `id` = " . $results["position"] . ";");
    if($sth != false){
        $results["position"] = mysql_fetch_assoc($sth);
    } else {
        http_response_code(404);
        die();
    }
...
function candidateListAll($link){
	$sth_1 = mysql_query("SELECT `id`, `firstName`, `lastName`, `party`, `position`, `shortDesc`, `img` FROM csc342.candidates WHERE 1", $link);
	if($sth_1 == false){
        http_response_code(404);
        die();
    }
    $rows = array();
        while($results = mysql_fetch_assoc($sth_1)) {
			...
			//Do add questions
			$sth = mysql_query("SELECT * FROM csc342.answers WHERE `candidateID` = " . $results["id"] . ";");
			if($sth != false){
				$results["QandA"] = array();
				while($r = mysql_fetch_assoc($sth)){
					$QnA["answer"] = $r["response"];
